...
import com.aftership.sdk.auth.CustomerSignature;
...
public class ApiRequestImpl implements ApiRequest {
  ...
  public <T, R> AftershipResponse<R> makeRequest(
    ...
    if (authenticationType == AuthenticationType.AES ||
      authenticationType == AuthenticationType.RSA) {
      requestHeaders.put("as-api-key", app.getApiKey());

      // add sign
      String date = DateTimeFormatter.RFC_1123_DATE_TIME.format(ZonedDateTime.now(ZoneOffset.UTC));
      CustomerSignature signature = CustomerSignature.getSignature(authenticationType, app.getApiSecret(), requestHeaders, url, method.getName(), reqBodyStr, "application/json; charset=utf-8", date);
      ...
      requestHeaders.put(signature.getHeader(), signature.getSignature());
    } else {
